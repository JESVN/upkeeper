...
Upkeep 是纯本机运行、不带账号的桌面工具，只在你点下「检查更新」时才发起版本检查所需的官方源请求，不做任何后台扫描。它做三件事：**看得见**（一屏看清当前版本 vs 最新版本）、**一键更新**（能无人值守的形态批量更新，带进度、失败隔离、代理注入与 UAC 提权）、**残留清理**（先出预览与字节账目，再按白名单删除）。

它把更新路径完全不同的东西放进同一套机制，不挑应用：

| 机制 | 例子 | 更新方式 | 无人值守 |
|---|---|---|---|
| 生态管理器 | winget、npm、pnpm、choco、uv、dotnet、pip、go… | 各自的官方命令 | ✅ |
| 自带更新器的 CLI | `omp`、`pi`、`codex`、`claude` | 工具自己的 `update` | ✅ |
| 桌面应用 | PiDeck、CC Switch、Clash Verge | 应用自己更新（winget 有清单的可走 winget） | ❌ 只检测提醒 |
| 绿色软件 | Beyond Compare、Apifox | 手动下载 | ❌ 只给下载页 |
| 命令式兜底（万能） | 任何有命令行的工具 | 你在配置里写的命令 | 取决于配置 |

加一个应用只改配置、不改代码；本机还没装的生态也只加一行管理器表。不知道从哪下手就点「发现应用」：它列出候选（含已装版本与证据），你勾选要纳管的即可。
...
- 🎛️ **什么都能纳管** —— winget / npm / pnpm / choco / uv / dotnet / pip 等生态、自带更新器的桌面应用、绿色软件，以及任何能用命令表达的方式；加应用只改配置，还能点「发现应用」让候选自己送上来。
- 🖐️ **一切由你发起** —— 不自动扫描、不后台轮询、不自动勾选：打开窗口只显示上次结果和它有多旧，更新目标永远是你勾中的那一行。
...
七个面板：**应用列表**（版本对比与状态徽标，单行失败不影响其它行）、**进度日志**（阶段、吞吐、ETA、退出码，失败行可一键开日志）、**残留清理**（逐规则给匹配数与字节数，确认后才执行）、**设置**（代理、并发、超时、保留策略、UAC 策略）、**历史**（每次动作一行 JSONL，只追加）、**通知**（跑完提示）、**添加应用**（发现候选 → 勾选纳管 → 只写用户级 `apps.yaml`）。界面规则与徽标定义见 [docs/ui.md](docs/ui.md)。
