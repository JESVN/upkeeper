...
- **`mod.rs` holds the trait, the `Form` enum, and the registry; the manager table lives in `managers.rs`.** A per-application branch anywhere in the core is a design error: the point of the mechanism model is that `core` never asks which mechanism it is talking to, and a new application is never a code change.
